...
def draw_map(xsamp,ysamp,data):
	...
	for i in range(img.size[0]):    # for every col:
		for j in range(img.size[1]):    # For every row
			if data[i][j]==0 :
				...
				pixels[i,j] = (23, 23, 254) # floor light grey
			elif data[i][j]==2:
				pixels[i,j] = (237, 230, 8) # threshold = yellow 
			elif data[i][j]==3:
				pixels[i,j] = (145, 0, 206) # small drop = purple  
			elif data[i][j]==4:
				pixels[i,j] = (132, 134, 134) # small item = dark grey 
			elif data[i][j]==5:
	...
	BLACK = (0, 0, 0, 0)
	draw.text((20, 20), "Laly's Map", fill=BLACK)
	img.show()
